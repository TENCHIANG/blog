...
* 类中如果没有写构造函数，编译器会加一个默认构造函数（修饰符只有 public）
* 如果手动写了构造函数，编译器就不会加默认构造函数了
...
* 次态（subtype）多态：以继承和接口实现的多态（父类型可以操作子类型）
...
* final 前面通常加上 static 也就是 static final，表示类的常量（属于类）
...
* **import static 静态导入**：从接口或类导入静态成员
* 名称冲突问题（从内到外，拆装箱？，多态）：
  * 优先使用**局部变量**
  * 优先使用类**成员**
  * 尝试[**重载**](jdk8note.md#重载与自动装箱)
  * 报错

```java
import static java.lang.System.out; // 注意要用完整名称
//import static java.lang.System.*; 注意名称冲突问题
out.println("aa");
```
...
* 数组是对象，所有对象都继承于 Object 类，所以通过 **Object 数组**可以实现可变参数
  ...
  * 可变参数就是数组的语法糖，Object... 类型的可变参数可以传不同类型的参数
  * 注意：可变参数只能是**最后一个参数**（**最多一个**可变参数）
...
* 定义在类区块中（成员内部类）或方法中（局部内部类）
* 作为成员的内部类
  * 内部类可以存取外部类的成员，一般申明为 private（辅助外部类操作）
  * 内部类也可声明为static，可存取外部类的 static 成员（将外部类当命名空间）
...
Some s = new Some();
Some.Other o = s.new Other(); // new看起来是Some的成员?
...
```java
// 匿名类继承Object类，且重写了toString()方法，最终实例化
Object o = new Object() {
