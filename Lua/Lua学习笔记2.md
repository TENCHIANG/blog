...
* 作为参数，只能放在最后（碰到**逗号**）（否则只有一个值）
* 不能用 **or** 连接两个返回多个值的函数（否则只有一个值）
* 用**括号**包起来，只有一个值（可以理解位括号只接收一个参数）
...
* 更**灵活**的函数编写方式
* 把每一类参数用花括号分类，每一类里面的参数都可以省略
* 函数定义是直接是可变参数
* 处理参数时用数组而不是select（麻烦）
* 如果参数有的分类有的不分类就需要分别处理（type，或者指定位置）

```lua
function f (...)
	for _, v in ipairs {...} do print(unpack(v)) end
	for i = 1, select("#", ...) do print(unpack((select(i, ...)))) end
end
...
* 无返回值是可以当作 nil 使用的，但是很多库函数一般都做了报错处理
  * 如 type() 的意义是？
  * 所以定义函数最好要返回东西

```lua
> function g () end -- 无返回值函数
> function f (...) print(select("#", ...)) end -- 无返回值能不能用在于函数怎么处理
> f(g()) -- 相当于f()
0
> f(nil)
1
